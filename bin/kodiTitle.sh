...
while [ true ]; do
	json=`curl -s --data-binary '{"jsonrpc": "2.0", "method": "Player.GetItem", "params": { "properties": ["title", "artist" ], "playerid": 0 }, "id": "AudioGetItem"}' -H 'content-type: application/json;' http://localhost:8080/jsonrpc`
